...
![Clawd approves](clawd_paper.png)
...
**Clawd는 화면에 실제로 무엇이 있는지를 읽고 반응하는 도트 공부 친구입니다.**

기존의 뽀모도로 타이머나 집중 모니터링 앱은 결국 스케줄러입니다. 정해진 시간에 정해진 문장을 출력할 뿐이라, 방금 증명 문제를 푼 사람에게도 한 시간 내내 딴짓한 사람에게도 똑같이 "잘하고 있어요!"라고 말합니다. 앱이 알 수 없으니 진심일 수 없고, 그래서 하루 만에 무시하게 됩니다.

Clawd는 그 반대를 지향합니다. 화면 맥락을 읽고, 그 내용을 근거로 **구체적인** 칭찬과 잔소리를 합니다. "12분째 딴짓 중"이 아니라 "3번 문제 풀다 말고 나갔잖아, 12분 지났어"라고 말합니다. 목표는 세 가지 감정입니다 — 세어지는 게 아니라 **보여지는** 데서 오는 양심, 실제 내용을 짚어주기에 증거가 되는 성취감, 그리고 작업이 끝난 뒤 확인하는 대시보드가 아니라 작업 *중에* 곁에 있는 캐릭터에서 오는 동지애.

퀴즈도 같은 이유로 만들었습니다. 보통은 Claude를 켜고, 화면을 캡처하고, 잘라 붙이고, 설명을 적어야 문제를 받을 수 있는데 — 그쯤이면 이미 공부 흐름이 끊깁니다. Clawd는 클릭 한 번으로 지금 화면의 내용에 대한 단답식 문제를 말풍선에 띄우고, 답도 말풍선에 받아서 채점과 해설까지 그 자리에서 끝냅니다.

**스크린샷 관련 주의.** 현재 기본 설정은 완전 수동이 아닙니다. 사용자가 요청할 때 외에도 공부 연속 3분, 딴짓 단계 상승, 딴짓 중 주기 재확인, 퀴즈 시작 시에 자동으로 촬영합니다. `api.screenshot_stray: false`로 딴짓 관련 자동 촬영은 끌 수 있지만, "요청할 때만" 되는 설정은 아직 없습니다 — 자세한 내용과 해결 계획은 위 [Privacy](#privacy)와 [Status](#status)에 적어두었습니다.

**개발 중입니다.** 동작하는 프로토타입이며 알려진 버그가 있습니다. 전체 설정 문서는 [docs/MANUAL.md](docs/MANUAL.md)를 봐주세요.

---

*A personal fan project. Not affiliated with, endorsed by, or connected to Anthropic.*
